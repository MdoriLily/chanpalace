...
$booking_query = $mysqli->query("select booking.*,sum(booking_room.total) as net_total
    ...
    where booking.booking_id=booking.booking_id{$query_status}
...
$status_success = 0;
$status_canceled = 0;
$booking_all_query = $mysqli->query("select * from booking where booking.name like '%{$search}%' or phone_no like '%{$search}%'");
                ...
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-md-end">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">รายการสั่งจอง</li>
                        </ol>
                    </nav>
                </div>
            </div>
                        ...
                        <tbody>
                            ...
                            while ($booking_result = $booking_query->fetch_assoc()) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <a href="booking_edit.php?id=<?php echo $booking_result['booking_id'];?>" class="btn btn-primary" title="แก้ไข"><i class="fa-solid fa-pen-to-square"></i></a>
                                            <a href="booking_delete_db.php?id=<?php echo $booking_result['booking_id'];?>" class="btn btn-danger" title="ลบ" onclick="return confirm('คุณต้องการลบรายการสั่งจองวันที่ <?php echo date_format(date_create($booking_result['created']),'d/m/Y');?> ของคุณ <?php echo $booking_result['name'];?> หรือไม่')"><i class="fa-solid fa-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                <?php
